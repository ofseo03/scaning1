...
**대상 브랜치는 신경 쓰지 않으셔도 됩니다.** `main`으로 PR을 열면
바꾼 파일에 맞는 기능 브랜치로 자동으로 옮겨집니다.

- 한 기능만 고쳤으면 → 그 기능 브랜치 (`feature/ocr`, `feature/writers` 등)
- 여러 기능에 걸쳐 있거나 문서·설정만 고쳤으면 → `main`

기능 브랜치에서 `main`으로 올리는 통합 PR은 그대로 둡니다.
자세한 규칙은 [docs/BRANCHING.md](docs/BRANCHING.md)에 있습니다.
