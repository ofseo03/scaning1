...
브라우저에서 바로 변환할 수 있는 화면이 있습니다.
사진이나 PDF를 끌어다 놓으면 `.docx`·`.hwpx` 로 바꿔 내려받습니다.

**[scaning1.vercel.app](https://scaning1.vercel.app)**

글자 인식(OCR)과 문서 만들기가 모두 브라우저 안에서 끝나므로
**파일이 어디로도 올라가지 않습니다.** 서버가 없으니 업로드도 없습니다.

다만 처음 한 번은 인식 엔진과 한국어 학습 데이터를 내려받느라 시간이 걸리고,
쪽수가 많으면 설치판보다 느립니다. 기울기 자동 보정·노이즈 제거·폴더 통째 변환은
설치판에만 있습니다.
...
| `feature/web` | 웹 (로컬 서버 + 브라우저판) |
| `feature/core` | 공통 뼈대 |

`main`으로 PR을 열면 바꾼 파일에 맞는 기능 브랜치로 **자동으로 옮겨집니다.**
한 기능만 고쳤으면 그 기능 브랜치로, 여러 기능에 걸치거나 문서만 고쳤으면 `main`에 그대로 둡니다.
`main`이 바뀌면 모든 기능 브랜치에 자동으로 되먹입니다.
